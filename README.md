# VR Lab Equipment Directory

An iPad-friendly directory linking to the VR Lab's standard Assetbots databases. The page provides concise instructions and five large destinations:
...
This repository is public. The Assetbots database URLs are therefore **not committed to this repository**. They are entered once on the lab iPad and stored in that browser's `localStorage`.
...
- Standard Assetbots database pages require authentication and inherit the signed-in account's permissions. Never leave an administrator session open on a shared device.
...
The recommended structure is one database with five asset categories. If Assetbots has explicitly authorised a legacy multi-database arrangement, this directory can instead link to each permitted database.

For each destination:

1. Open the relevant Assetbots database.
2. Copy its normal HTTPS address from the browser address bar.
3. Test the address in a private browser and confirm that Assetbots requests authentication.
4. Decide which authorised accounts may use each database and apply the minimum necessary permissions.
5. Do not configure a shared iPad with an administrator session.
...
2. Paste the five tested standard Assetbots database URLs and select **Save and open homepage**.
3. Open each tile and confirm that it reaches the intended database and requests the expected authentication.
...
## Instructions for authorised users

1. Select the correct equipment group.
2. Sign in with your authorised account if prompted.
3. Scan the item's QR label or search by asset tag.
4. Check the record carefully before making a permitted change.
5. Sign out when finished if the device is shared.

If the correct asset is missing or access is denied, stop and ask a VR Lab administrator rather than selecting a similar record or using another person's account.
...
Before deployment, test portrait and landscape layouts, 200% browser zoom, keyboard navigation, VoiceOver, online/offline status, authentication, every destination and the route back to the homepage.
